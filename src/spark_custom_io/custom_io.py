...
class CustomWriter(DataSourceWriter):
    def __init__(self, options):
        ...
        makedirs(self.rootPath, exist_ok=True)
...
class CustomReader(DataSourceReader):
    ...
    def partitions(self):
        ...
        with open(self.inputPath + "/.metadata.json", "r") as f:
            metadata = load(f)
        return [
            InputPartition({"start": 0, "end": 1, "idx": idx})
...
if __name__ == "__main__":
    ...
    df.write.format("customconsole").mode("append").option("prefix", "LOG:").option(
        "outputPath", "/Volumes/workspace/testing_schema/testing_volumne/t1"
    ).save()
